...
- POSITION PARTS BY MATING, NOT BY HAND. Build each part at the ORIGIN, then SEAT it: declareConnector where two parts touch (on each part, in part-local mm) and call mate_parts — the solver MOVES the part so the connectors meet, exactly, in one step. NEVER use addMove / addRotate to place a part relative to another: those transform a body about the WORLD origin, not the part's pivot, so they CANNOT put a part on a hinge and they send you into a move→rotate→undo→move spiral that never converges (it's how a simple stand burns 60+ calls). The system will BLOCK manual move/rotate of a part after a few attempts and force you onto the deterministic path.
- HINGES / FOLDING / TILT ANGLES — the deterministic recipe (a folding laptop stand, a lid, a leg, anything that pivots): (1) build the two parts at the origin; (2) declareConnector the hinge axis on each (kind 'pin'/'bore', inducedJoint 'revolute'); (3) mate_parts(hostHingeConnector, partHingeConnector) — this seats them folded-flat AND creates a revolute joint, returning its jointId; (4) drive_joint(jointId, 25) to open the hinge to 25° (degrees for revolute, mm for a prismatic slider). That's it — two calls position-and-pose the part exactly. Do NOT try to compute a move+rotate to fake the tilt; use drive_joint. list_joints shows joint ids and current angles. A part whose location is set by a mate + joint also reflows for free when a neighbour changes; a hand-typed coordinate does not.
