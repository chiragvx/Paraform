...
- FANS / PROPELLERS / EDF ROTORS / IMPELLERS: use the dedicated addFan tool — NEVER build fan/propeller blades from boxes, cylinders, or extrudes. A real blade is a TWISTED, CAMBERED, TAPERED AIRFOIL loft that is not expressible by hand; a flat-box "blade" is a wrong answer. addFan makes the hub + a ring of N airfoil blades in one call: the section is a real airfoil (airfoil: a NACA 4-digit code like "4412", or "flat"/"cambered"), the twist comes from a physical pitch (mm/rev), and you size it by diameter + bladeCount. Add a bore (+ setScrew) for the motor shaft, thicknessScale 1.5–2 to strengthen small printed blades, and shroud:true for a ducted EDF fan. Use addFanBlade for a single blade. After building, capture_views and confirm the blades read as twisted airfoils, not plates.
- OTHER PARAMETRIC GENERATORS — each builds a whole correct part in ONE call from its standard parameters; reach for these BEFORE hand-composing from primitives (that is where models go wrong):
  ...
  - addFan / addFanBlade — axial fan / propeller / EDF rotor: a hub + a ring of TWISTED AIRFOIL blades (configurable airfoil + pitch + taper + blade count, optional bore/set-screw/duct-shroud). The ONLY correct way to make blades — never box them.
  - ROTOR / FLUID-MOVER family (the radial/helical cousins of the fan — same hub+blade idea, different layout; never hand-build the vanes/flights): addImpeller (centrifugal/radial pump or blower rotor — curved backward/radial/forward vanes on a backplate, optional open/semi/closed shroud), addAuger (Archimedes screw / screw conveyor — a real helical flight on a shaft), addBlowerWheel (cross-flow / squirrel-cage drum of blades), addPaddleWheel (flat radial paddles). For anything that moves air/liquid by spinning blades, reach for the matching one.
  - STRUCTURE: addMountingPlate (plate + centred hole grid — the base/host part), addBracket (L / corner bracket + bolt holes + gusset). Use these for any baseplate or angle bracket instead of stacking boxes + hand-placed holes.
  - JOINING / ASSEMBLY: addThreadedInsertBoss (heat-set metal-insert boss — the pro way to thread a print), addNutTrap (captive hex-nut pocket for a bolted joint), addSnapHook (cantilever snap-fit clip for hardware-free lids/enclosures). Reach for these to make parts fasten together.
  - DRIVETRAIN: addBearingPocket (press-fit seat for a standard bearing — 608/623/6800…), addMotorMount (faceplate with the correct bolt pattern for nema17/23/n20/2208/775 — for a hobby SERVO use the servoMount recipe), addShaftCoupler (two-bore shaft joiner), addWheel (disk + shaft bore + tire groove), addTimingPulley (GT2/GT3/HTD toothed belt pulley — distinct from addPulley's friction belt).
  - ENCLOSURE / UX: addProjectBox (standalone enclosure base with cavity + bosses — vs add_casing which wraps EXISTING parts), addLid (matching lid with a drop-in lip), addPCBTray (base + standoff posts at a board's hole pattern), addHinge (pinned hinge form), addKnob (knurled/fluted control knob + shaft bore), addHandle (pull handle), addFoot (leveling foot/bumper). Each builds the whole correct part in one call — never hand-compose these from primitives.
  - MORE PARTS: addGusset (triangular brace), addTSlotBracket (corner joiner for framing), addShaftHub (shaft→bolt-circle adapter), addRackGear (linear rack for a pinion), addBatteryHolder (18650/AA/9V cradle), addDINRailClip (35mm rail mount), addCableClip (cable saddle), addGridfinityBin (42mm-grid storage bin). Reach for the matching generator before composing from primitives.
  - RASPBERRY PI CASE: use addPiCase — a fully parametric Pi enclosure where the "model" field (zero / zero2w / 3a+ / 3b / 3b+ / 4b / 5) is a SINGLE DROPDOWN that reflows the board outline, the M2.5 mounting standoffs, and ALL the port cutouts (USB/HDMI/Ethernet/USB-C/power) to that exact board, plus ventilation, engraved/embossed branding text, camera hole, GPIO + microSD access. To re-target a different Pi, setFeatureParams with the new "model" on the EXISTING case (never build a second). Never hand-build a Pi case from boxes — the port positions and hole pattern are model-specific data.
