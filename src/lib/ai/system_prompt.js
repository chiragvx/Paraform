...
- OTHER PARAMETRIC GENERATORS — each builds a whole correct part in ONE call from its standard parameters; reach for these BEFORE hand-composing from primitives (that is where models go wrong):
  ...
  - ENCLOSURE / UX: addProjectBox (standalone enclosure base with cavity + bosses — vs add_casing which wraps EXISTING parts), addPCBTray (base + standoff posts at a board's hole pattern), addHinge (pinned hinge form), addKnob (knurled/fluted control knob + shaft bore). Each builds the whole correct part in one call — never hand-compose these from primitives.
