<script>
   ...
   * build status, with the physics-closure result, version history, and
   * checkpoint revert. The graph is mutated by the AI via plan_* tools; this
   * panel is the human's window + steering surface.
   */
  import { planState, refreshPlan, runPlanClosure } from '$lib/ai/plan/plan_store.svelte.js';
...
{#snippet nodeRow(node, depth)}
  <div class="flex items-center gap-1.5 py-0.5 text-xs" style="padding-left:{depth * 12}px">
    ...
    {#if node.kind === 'part' || node.kind === 'instance'}
