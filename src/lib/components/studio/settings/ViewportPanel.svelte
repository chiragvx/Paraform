<script>
  ...
  const panel = SETTINGS_SCHEMA.find((p) => p.id === 'viewport');

  // Latest panel values, kept reactive so a refresh of `values` from outside
  // (e.g. dialog re-open re-hydrating from storage) still flows through.
  const live = $derived(values);

  function update(key, v) {
    values[key] = v;
    saveSetting('viewport', key, v);
    // Broadcast the full panel state to the running renderer so the change
    // shows up immediately. Safe-noop if the viewport hasn't booted yet.
    studio.viewport?.applyViewportSettings?.({ ...live });
  }
</script>
