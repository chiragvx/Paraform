<script>
  ...
  import { setTheme } from '$lib/theme/theme.svelte.js';

  let { values = $bindable({}) } = $props();

  const panel = SETTINGS_SCHEMA.find((p) => p.id === 'general');

  function update(key, v) {
    values[key] = v;
    // `theme` is special — setTheme both persists AND applies the change
    // live (toggling the `.dark` class on <html>). Other fields just save.
    if (key === 'theme') setTheme(v);
    else saveSetting('general', key, v);
  }
</script>
