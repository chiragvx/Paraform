...
export function emitPiCasePython(f) {
    ...
    // ----- assemble -----
    // 'both' shows TWO SEPARATE parts (Body + Lid). The lid is exploded above the
    // open base so the lip never overlaps the base — otherwise a single boolean
    // FUSES them into one solid (then the Pi can't go in and the lid won't lift).
    // compoundFix keeps the two disjoint solids as a 2-part Compound. Generate
    // part:'base' or part:'lid' to print each one flat.
    const explode = Number(p.explode) >= 0 ? Number(p.explode) : (lipH + Math.max(10, headroom * 0.5));
    L.push(`try:`);
    if (part === 'base') {
        ...
        L.push(`    n_${id}=_base_${id}()+_lid_${id}().moved(Location((0,0,${C(baseH + explode)})))`);
    }
    L.push(`except Exception:`);
