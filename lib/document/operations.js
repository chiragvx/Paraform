...
import { resolvePiModel } from './generators.js';
import {
...
/**
 * Owning component for a generated part. A generated body emits at the world
 * origin, so to be *seatable* by a mate it needs its own component — the mate
 * solver repositions a COMPONENT (via setComponentOrigin), not a raw feature.
 * Reuse the caller's componentId when supplied; otherwise spin up a fresh one.
 * Connectors the generator stamps are parented here so they ride the part when
 * it is mated. See src/lib/library/place.js `mateConnectors` and CLAUDE.md
 * "The connector contract".
 */
function _generatedPartComponent(i, name) {
    return (i && typeof i.componentId === 'string' && i.componentId)
        ? i.componentId
        : addComponent({ name, parentId: 'root' }).id;
}
...
export function addBatteryHolder(i = {}) {
    ...
    const componentId = _generatedPartComponent(i, `${params.cellCount}× ${params.cellType} holder`);
    const f = _mkFeature('BatteryHolder', params, {}, { componentId });
    getDocumentStore().commit(addFeatureChange(f));
    // Side-mount face. A battery pack mounts OUTSIDE the enclosure — the interior
    // is for the PCB — flush against a wall and sitting on the ground beside it.
    // The holder block is XY-centred / Align.MIN on Z, so its -Y face is at
    // -blkL/2; axis −Y points OUT of that face (connector contract rule 3). Cell
    // length matches the emitter's CELL_MM so the face lands on real geometry.
    const _CELL = { '18650': [18.6, 65], '21700': [21.2, 70], aa: [14.6, 50.5], aaa: [10.6, 44.5], c: [26.2, 50], '9v': [26.5, 48.5] };
    const _cell = _CELL[String(params.cellType).toLowerCase()] || _CELL['18650'];
    const blkL = _cell[1] + 2 * params.wall;
    addConnector({
        parent: componentId, kind: 'planar', gender: 'male',
        size: { nominal: 'unspecified', unit: 'mm' },
        origin: [0, -blkL / 2, 0], axis: [0, -1, 0], normal: [0, 0, 1],
        inducedJoint: 'fixed',
        interfaceId: 'enclosure-side-v1', role: 'accessory-mount-face', locked: true,
        metadata: { source: 'generator', generator: 'addBatteryHolder' },
    });
    return f;
}
...
/**
 * Model-parametric Raspberry Pi case — `model` is the single dropdown that
 * reflows board size, mounting-hole standoffs, and all port cutouts. Adds
 * ventilation, branding text, camera/IO/GPIO/microSD access; emits base / lid /
 * both. See emitPiCasePython + PI_MODELS for the per-model mechanical data.
 */
export function addPiCase(i = {}) {
    const model = (typeof i.model === 'string' && i.model.trim()) ? i.model.trim() : '5';
    const m = resolvePiModel(model);
    const params = {
        model,
        part: ['base', 'lid', 'both'].includes(i.part) ? i.part : 'both',
        explode: Number(i.explode) >= 0 ? Number(i.explode) : -1,   // -1 = auto (lid floats above the base)
        wall: _num(i.wall, 2), clearance: Number(i.clearance) >= 0 ? Number(i.clearance) : 0.6,
        standoffHeight: _num(i.standoffHeight, 4), headroom: _num(i.headroom, 0),
        ventilation: ['none', 'slots', 'grid'].includes(i.ventilation) ? i.ventilation : 'slots',
        brandingText: typeof i.brandingText === 'string' ? i.brandingText : (m.label || 'Raspberry Pi'),
        brandingEngrave: i.brandingEngrave !== false,
        cameraHole: i.cameraHole === true, cameraDiameter: _num(i.cameraDiameter, 8),
        gpioAccess: i.gpioAccess === true, microSDAccess: i.microSDAccess !== false,
        ioCutouts: i.ioCutouts !== false, lidScrews: i.lidScrews === true,
    };
    const componentId = _generatedPartComponent(i, `${m.label || 'Raspberry Pi'} case`);
    const f = _mkFeature('PiCase', params, {}, { componentId });
    // surface the resolved model so a mismatch is visible
    f.warnings = [...(f.warnings || [])];
    if (!Object.prototype.hasOwnProperty.call({ zero: 1, zero2w: 1, '3a+': 1, '3b': 1, '3b+': 1, '4b': 1, '5': 1 }, model.toLowerCase()))
        f.warnings.push(`Pi case: model "${model}" resolved to "${m.label}". Known models: zero, zero2w, 3a+, 3b, 3b+, 4b, 5.`);
    if (!f.warnings.length) delete f.warnings;
    getDocumentStore().commit(addFeatureChange(f));
    if (params.part !== 'lid') {
        const wall = Number(params.wall) > 0 ? Number(params.wall) : 2;
        const clr = Number(params.clearance) >= 0 ? Number(params.clearance) : 0.6;
        const bW = Array.isArray(m.board) ? m.board[1] : 56;
        const outerHY = bW / 2 + clr + wall;
        // Interior floor — where the Pi BOARD mounts. RESERVED for the board:
        // batteries / accessories must NOT seat here (they'd clash with the PCB
        // that goes inside). Distinct interface so a battery can't match it.
        addConnector({
            parent: componentId, kind: 'planar', gender: 'female',
            size: { nominal: 'unspecified', unit: 'mm' },
            origin: [0, 0, wall], axis: [0, 0, 1], normal: [1, 0, 0],
            inducedJoint: 'fixed',
            interfaceId: 'pi-board-floor-v1', role: 'board-floor', locked: true,
            metadata: { source: 'generator', generator: 'addPiCase' },
        });
        // Exterior side mount — batteries / accessories attach to the OUTSIDE of
        // the long (+Y) wall and sit on the ground BESIDE the case, leaving the
        // interior free for the board. axis +Y points out of the wall.
        addConnector({
            parent: componentId, kind: 'planar', gender: 'female',
            size: { nominal: 'unspecified', unit: 'mm' },
            origin: [0, outerHY, 0], axis: [0, 1, 0], normal: [0, 0, 1],
            inducedJoint: 'fixed',
            interfaceId: 'enclosure-side-v1', role: 'accessory-mount', locked: true,
            metadata: { source: 'generator', generator: 'addPiCase' },
        });
    }
    return f;
