...
import { resolvePiModel, PI_MODELS } from '../generators.js';
...
import { sceneDigest, documentSummary, AGENT_TOOLS } from '../../../src/lib/ai/tools.js';
import { mateConnectors } from '../../../src/lib/library/place.js';
import { connectorsCompatible } from '../../../src/lib/library/mate_solver.js';
import { selectAgentTools } from '../../../src/lib/ai/agent.js';
...
await t('GENERATOR_TOOLS exposes every generator in the standard shape', () => {
    ...
    assert.equal(names.length, 37, `expected 37 generators, got ${names.length}`);
...
const P0 = [
    ...
    ['addPiCase', addPiCase, { model: '5', part: 'both' }, 'PiCase', /raspberry pi|pi 5/i],
];
...
// ── Raspberry Pi case — the "single dropdown reflows everything" requirement ────
await t('Pi case: model dropdown reflows board size (Pi 5 = 85×56, Zero = 65×30)', () => {
    assert.deepEqual(resolvePiModel('5').board, [85, 56]);
    assert.deepEqual(resolvePiModel('4b').board, [85, 56]);
    assert.deepEqual(resolvePiModel('zero').board, [65, 30]);
    assert.deepEqual(resolvePiModel('3a+').board, [65, 56]);
    assert.equal(resolvePiModel('garbage').label, 'Pi 5');          // forgiving default
    assert.equal(resolvePiModel('pi4').label, 'Pi 4B');             // alias
});

await t('Pi case: every model emits a body, with model-specific port count + hole spacing', () => {
    for (const key of Object.keys(PI_MODELS)) {
        resetDocumentStore();
        const f = addPiCase({ model: key, part: 'base' });
        assert.equal(f.type, 'PiCase');
        const { code } = emitDocument(getDocumentStore().doc);
        assert.ok(!/unknown feature type/.test(code) && !/emit failed for/.test(code), `${key} emit`);
        assert.ok(code.includes(`n_${f.id}`), `${key} body`);
        // the emitted code names this model's ports (model-specific cutouts)
        for (const port of PI_MODELS[key].ports.slice(0, 2)) assert.ok(code.includes(port.name), `${key} missing ${port.name}`);
    }
});

await t('Pi case: changing model is ONE edit in place (no second case) and changes the geometry', () => {
    resetDocumentStore();
    const f = addPiCase({ model: '5', part: 'both' });
    assert.equal(documentSummary().featureCount, 1);
    const pi5Code = emitDocument(getDocumentStore().doc).code;
    setFeatureParams(f.id, { model: 'zero' });
    assert.equal(documentSummary().featureCount, 1, 'switching model must NOT add a second case');
    const zeroCode = emitDocument(getDocumentStore().doc).code;
    assert.notEqual(pi5Code, zeroCode, 'model change must reflow the emitted geometry');
    // Pi 5 has an Ethernet cutout; the Zero does not → proves the ports reflow
    assert.ok(pi5Code.includes('Ethernet') && !zeroCode.includes('Ethernet'), 'ports did not reflow with the model');
    assert.match(sceneDigest(), /Pi zero|raspberry pi zero/i);
});

await t('Pi case: part="both" is TWO separate parts — lid exploded above the base (not fused)', () => {
    resetDocumentStore();
    const f = addPiCase({ model: '5', part: 'both' });
    const { code } = emitDocument(getDocumentStore().doc);
    // 'both' unions base + lid but lifts the lid clear so OCCT can't fuse them
    const lid = code.match(/_lid_\w+\(\)\.moved\(Location\(\(0,0,([\d.]+)\)\)\)/);
    assert.ok(lid, 'both must place the lid');
    assert.ok(parseFloat(lid[1]) > 30, `lid must be exploded above the base (got z=${lid && lid[1]}), else it fuses into one solid`);
    // base alone: only the base builder, no lid translation
    resetDocumentStore();
    const b = addPiCase({ model: '5', part: 'base' });
    const baseCode = emitDocument(getDocumentStore().doc).code;
    assert.ok(baseCode.includes(`_base_${b.id}()`) && !/_lid_\w+\(\)\.moved/.test(baseCode), 'base part must not include the lid');
});

await t('Pi case: branding/vent/camera options are honoured in the emit', () => {
    resetDocumentStore();
    const f = addPiCase({ model: '4b', part: 'lid', brandingText: 'NOVA', cameraHole: true, gpioAccess: true });
    const { code } = emitDocument(getDocumentStore().doc);
    assert.match(code, /Text\("NOVA"/);             // branding
    assert.ok(code.includes('camera hole') || /Cylinder\(.*camera/i.test(code) || /camera/i.test(code), 'camera hole emitted');
});

// Pi 5: outerHY = 56/2 + 0.6 + 2 = 30.6 ; battery 18650 blkL/2 = (65 + 2*2)/2 = 34.5
const BESIDE_Y = 30.6 + 34.5;   // 65.1 — holder centre sits OUTSIDE the +Y wall

await t('Pi case + battery holder: holder mounts BESIDE the case (interior reserved for the board), on the ground', () => {
    resetDocumentStore();
    addPiCase({ model: '5', part: 'both' });
    addBatteryHolder({ cellType: '18650', cellCount: 2 });
    assert.equal(documentSummary().featureCount, 2, 'two features (a component is not a feature)');

    const doc = getDocumentStore().doc;
    const conns = Object.values(doc.connectors || {});
    // The case reserves the INTERIOR floor for the board (distinct interface) and
    // exposes a separate exterior side-mount for accessories.
    const boardFloor = conns.find((c) => c.role === 'board-floor' && c.interfaceId === 'pi-board-floor-v1');
    const caseConn = conns.find((c) => c.role === 'accessory-mount' && c.interfaceId === 'enclosure-side-v1');
    const holderConn = conns.find((c) => c.role === 'accessory-mount-face' && c.interfaceId === 'enclosure-side-v1');
    assert.ok(boardFloor, 'case reserves an interior board-floor connector for the PCB');
    assert.ok(caseConn, 'case exposes an exterior accessory side-mount');
    assert.ok(holderConn, 'battery holder exposes a side-mount face');
    // The battery must NOT be able to mate the board floor (that is the PCB spot).
    assert.ok(!connectorsCompatible(boardFloor, holderConn), 'battery is incompatible with the board floor — it cannot land in the PCB spot');

    const r = mateConnectors(caseConn.id, holderConn.id);
    assert.ok(r.ok, `mate failed: ${r.error || ''}`);
    const seated = getDocumentStore().doc.components[holderConn.parent].origin;
    const [x, y, z] = seated.position;
    assert.ok(Math.abs(x) < 1e-6, 'holder centred in X');
    assert.ok(Math.abs(z) < 1e-6, `holder sits on the ground z=0, got z=${z}`);
    assert.ok(Math.abs(y - BESIDE_Y) < 1e-6, `holder seats flush BESIDE the case at y≈${BESIDE_Y}, got y=${y}`);
    assert.ok(y > 30.6, 'holder is OUTSIDE the case footprint (not inside the cavity)');
    assert.ok(Math.abs(seated.rotation[2]) < 1e-6, 'side mount needs no roll');
    assert.ok(r.mateId && getDocumentStore().doc.mates[r.mateId], 'a mate record persists');
});

await t('addBatteryHolder TOOL auto-seats BESIDE an existing Pi case (weak-model safety net — no manual mate needed)', () => {
    resetDocumentStore();
    addPiCase({ model: '5', part: 'both' });                 // host exists first
    const tool = GENERATOR_TOOLS.find((x) => x.name === 'addBatteryHolder');
    const res = tool.handler({ cellType: '18650', cellCount: 2, wall: 2 });
    assert.ok(res.ok, 'handler ok');
    assert.match(res.summary, /auto-seated into/, 'summary reports the auto-seat');

    const doc = getDocumentStore().doc;
    const holderConn = Object.values(doc.connectors).find((c) => c.role === 'accessory-mount-face');
    const o = doc.components[holderConn.parent].origin;
    assert.ok(Math.abs(o.position[1] - BESIDE_Y) < 1e-6, `holder auto-seated beside the case at y≈${BESIDE_Y}`);
    assert.ok(Math.abs(o.position[2]) < 1e-6, 'on the ground z=0');
    assert.equal(Object.keys(doc.mates || {}).length, 1, 'exactly one mate recorded');
});

await t('REVERSE order: holder built FIRST, then the Pi case absorbs it (order-independent auto-assembly)', () => {
    resetDocumentStore();
    const holderTool = GENERATOR_TOOLS.find((x) => x.name === 'addBatteryHolder');
    const caseTool = GENERATOR_TOOLS.find((x) => x.name === 'addPiCase');
    const rh = holderTool.handler({ cellType: '18650', cellCount: 2, wall: 2 });   // no host yet
    assert.doesNotMatch(rh.summary, /auto-/, 'holder alone has nothing to seat into');
    const rc = caseTool.handler({ model: '5', part: 'both', wall: 2 });            // case pulls it in
    assert.match(rc.summary, /auto-seated .*holder into it/, 'case absorbs the existing holder');

    const doc = getDocumentStore().doc;
    const holderConn = Object.values(doc.connectors).find((c) => c.role === 'accessory-mount-face');
    const o = doc.components[holderConn.parent].origin;
    assert.ok(Math.abs(o.position[1] - BESIDE_Y) < 1e-6, 'holder ends up beside the case regardless of build order');
    assert.equal(Object.keys(doc.mates || {}).length, 1, 'exactly one mate');
});

await t('addBatteryHolder TOOL with NO host present leaves the part at origin (no spurious mate)', () => {
    resetDocumentStore();
    const tool = GENERATOR_TOOLS.find((x) => x.name === 'addBatteryHolder');
    const res = tool.handler({ cellType: '18650', cellCount: 2 });
    assert.ok(res.ok);
    assert.doesNotMatch(res.summary, /auto-seated/, 'nothing to seat into → no auto-seat');
    assert.equal(Object.keys(getDocumentStore().doc.mates || {}).length, 0, 'no mate recorded');
});

await t('mateConnectors rejects incompatible connectors (no silent mis-seat)', () => {
    resetDocumentStore();
    const kase = addPiCase({ model: '5', part: 'both' });
    const holder = addBatteryHolder({ cellType: '18650', cellCount: 2 });
    const doc = getDocumentStore().doc;
    const conns = Object.values(doc.connectors || {});
    const caseConn = conns.find((c) => c.role === 'accessory-mount');
    // A bogus id must fail cleanly rather than throw or move the wrong thing.
    const bad = mateConnectors(caseConn.id, 'cnx_does_not_exist');
    assert.equal(bad.ok, false);
    assert.match(bad.error, /unknown part connector/);
});

await t('per-turn tool triage: irrelevant generators are dropped, relevant ones kept, surface shrinks', () => {
    const all = AGENT_TOOLS;
    const sel = selectAgentTools(all, 'make a raspberry pi case with a battery holder');
    const names = new Set(sel.map((s) => s.name));
    assert.ok(names.has('addPiCase'), 'addPiCase kept (request mentions "case")');
    assert.ok(names.has('addBatteryHolder'), 'addBatteryHolder kept (mentions "battery"/"holder")');
    assert.ok(names.has('mate_parts') && names.has('placeLibraryPart'), 'assembly tools always on');
    assert.ok(!names.has('addImpeller') && !names.has('addAuger') && !names.has('addPaddleWheel'), 'unrelated generators dropped');
    assert.ok(sel.length < all.length, `surface shrank (${sel.length} < ${all.length})`);
    // A request that names no generator still keeps the whole non-generator surface.
    const bare = selectAgentTools(all, 'fillet that edge');
    assert.ok(bare.some((s) => s.name === 'mate_parts'), 'non-generators unaffected by triage');
});
