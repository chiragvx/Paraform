...
import {
    makeSketchData, addEntity, stockPlane, facePlane,
...
suite('sketches', () => {
    ...
    test('offset stock plane emits Plane.XY.offset(n) — profile lands ON a body, not the origin', () => {
        const sk = makeSketchData(stockPlane('XY', 20));
        const c = makePoint(0, 0); addEntity(sk, c);
        addEntity(sk, makeCircle(c.id, 4));
        const feature = makeSketchFeature(sk);
        const doc = buildDoc(addFeatureChange(feature));
        const { code } = emitDocument(doc);
        assert.match(code, new RegExp(`with BuildSketch\\(Plane\\.XY\\.offset\\(20\\)\\) as sk_${feature.id}:`));
    });

    test('zero offset is omitted (stays a bare principal plane)', () => {
        const sk = makeSketchData(stockPlane('XY', 0));
        const c = makePoint(0, 0); addEntity(sk, c);
        addEntity(sk, makeCircle(c.id, 4));
        const feature = makeSketchFeature(sk);
        const { code } = emitDocument(buildDoc(addFeatureChange(feature)));
        assert.match(code, new RegExp(`with BuildSketch\\(Plane\\.XY\\) as sk_${feature.id}:`));
    });

    test('face-anchored sketch emits an inline Plane(origin, z_dir) at the picked face', () => {
        const desc = { kind: 'face', feature: 'box_x', opTag: 'box', part: '+Z', parents: [] };
        const sk = makeSketchData(facePlane(desc, [0, 0, 20], [0, 0, 1]));
        const c = makePoint(0, 0); addEntity(sk, c);
        addEntity(sk, makeCircle(c.id, 3));
        const feature = makeSketchFeature(sk);
        const { code } = emitDocument(buildDoc(addFeatureChange(feature)));
        assert.match(code, /with BuildSketch\(Plane\(origin=Vector\(0, 0, 20\), z_dir=Vector\(0, 0, 1\)\)\)/);
    });

    test('Plane feature: offset is honoured (regression — the offset used to be dropped)', () => {
        const plane = makeFeature('Plane', { type: 'offset', basePlane: 'XY', offset: 15 });
        const { code } = emitDocument(buildDoc(addFeatureChange(plane)));
        assert.match(code, new RegExp(`pl_${plane.id} = Plane\\.XY\\.offset\\(15\\)`));
    });
});
...
suite('executor', () => {
    ...
    test('lastResult() / lastError() reflect the most recent outcome', async () => {
        ...
        const exec = new DocumentExecutor({ client });
        // Two DISTINCT docs so each compile is a real kernel call. The result
        // cache (keyed on emitted code) would otherwise replay the first
        // success for a re-run of the same doc — re-running identical code is
        // exactly the redundant compile the cache is built to short-circuit.
        const doc1 = buildDoc(addFeatureChange(makeFeature('Box')));
        const doc2 = buildDoc(addFeatureChange(makeFeature('Cylinder')));
        await exec.executeDocument(doc1);
        assert.equal(exec.lastResult().glb, 'GLB1');
        await exec.executeDocument(doc2);
...
// ── BuildScript (raw/AI code → sandboxed body) ───────────────────────────────
suite('buildscript', () => {
    test('emits n_<id> = _bs_run(<code literal>, <label>) — sandboxed, not inline', () => {
        const code = 'from build123d import *\nresult = Box(1, 2, 3)';
        const f = makeFeature('BuildScript', { code });
        f.name = 'MyScript';
        const doc = buildDoc(addFeatureChange(f));
        const { code: emitted } = emitDocument(doc);
        // Routed through the kernel sandbox helper, with the source as a Python
        // string literal (JSON-encoded) and the feature name as the label.
        assert.match(emitted, new RegExp(`n_${f.id} = _bs_run\\(`));
        assert.ok(emitted.includes(JSON.stringify(code)), 'source embedded as a Python string literal');
        assert.ok(emitted.includes('"MyScript"'), 'feature name passed as the label');
        // The old behaviour dumped the raw code inline — make sure we no longer do.
        assert.ok(!emitted.includes('\nfrom build123d import *\nresult = Box'),
            'raw code must NOT be spliced inline into the namespace');
    });

    test('a BuildScript is a renderable leaf (its result body is collected)', () => {
        const f = makeFeature('BuildScript', { code: 'result = None' });
        const doc = buildDoc(addFeatureChange(f));
        const { leafIds } = emitDocument(doc);
        // BuildScript is no longer excluded from leafIds — its n_<id> is a body.
        assert.deepEqual(leafIds, [f.id]);
    });

    test('special characters in the script survive as a valid string literal', () => {
        // Quotes, backslashes and newlines must not break the emitted Python.
        const code = 'r = "a\\tb"  # comment with \'quotes\' and \\ backslash\nresult = None';
        const f = makeFeature('BuildScript', { code });
        const { code: emitted } = emitDocument(buildDoc(addFeatureChange(f)));
        assert.ok(emitted.includes(JSON.stringify(code)), 'code round-trips through py() intact');
    });
});
