...
import { featureDepHashes, hashesToObject } from './dep_hash.js';
...
import {
    ...
    emitMountingPlatePython, emitBracketPython, emitInsertBossPython,
    emitNutTrapPython, emitSnapHookPython, emitBearingPocketPython,
    emitMotorMountPython, emitShaftCouplerPython, emitWheelPython,
    emitTimingPulleyPython, emitHingePython, emitProjectBoxPython,
    emitPCBTrayPython, emitKnobPython,
    emitFootPython, emitGussetPython, emitHandlePython, emitShaftHubPython,
    emitLidPython, emitRackGearPython, emitBatteryHolderPython, emitDINRailClipPython,
    emitCableClipPython, emitGridfinityBinPython, emitTSlotBracketPython,
    emitImpellerPython, emitAugerPython, emitBlowerWheelPython, emitPaddleWheelPython,
    emitPiCasePython,
} from './generators.js';
...
const EMITTERS = {
    ...
    BuildScript(f) {
        ...
        // Untrusted user/AI code does NOT run inline in the shared namespace —
        // the kernel's `_bs_run` (harness.py) execs it in a restricted sandbox
        // (geometry/math imports only, no filesystem/network/process, timeout)
        // in its OWN globals, so multiple scripts can't collide on names. It
        // returns whatever the script bound to `result`; we assign that to this
        // feature's leaf body var `n_<id>`, so a non-None result RENDERS (the
        // feature is a leaf — see the leafIds filter) and a helper-only script
        // (no `result`) yields None and is skipped downstream. `py()` emits a
        // safe Python string literal of the source, so no escaping/indentation
        // games are needed.
        return `n_${f.id} = _bs_run(${py(code.toString())}, ${py(f.name || 'Script')})`;
    },
    ...
    /**
     * Reference image (Phase A1). Scene-only underlay the user traces against —
     * the kernel never participates. Emit a comment only (no `n_<id>`); the
     * feature is excluded from the leaf-body set below so a doc with reference
     * images compiles to byte-identical Python as one without them.
     */
    ReferenceImage(f) {
        const { name = '?' } = f.params || {};
        return `# Reference image: ${name} — JS-side underlay, no kernel body`;
    },
    ...
    Gear(f) {
        return emitGearPython(f);
    },

    // ── Parametric domain generators (power transmission / framing / mounting) ──
    // Each is a pure, fail-safe emitter in ./generators.js; see that file's
    // header for the contract. They exist so a weak model places one node
    // ("a pulley") instead of hand-composing it from primitives.
    Pulley(f) { return emitPulleyPython(f); },
    Sprocket(f) { return emitSprocketPython(f); },
    TSlotExtrusion(f) { return emitTSlotPython(f); },
    ScrewBoss(f) { return emitScrewBossPython(f); },
    Standoff(f) { return emitStandoffPython(f); },
    Fan(f) { return emitFanPython(f); },
    MountingPlate(f) { return emitMountingPlatePython(f); },
    Bracket(f) { return emitBracketPython(f); },
    ThreadedInsertBoss(f) { return emitInsertBossPython(f); },
    NutTrap(f) { return emitNutTrapPython(f); },
    SnapHook(f) { return emitSnapHookPython(f); },
    BearingPocket(f) { return emitBearingPocketPython(f); },
    MotorMount(f) { return emitMotorMountPython(f); },
    ShaftCoupler(f) { return emitShaftCouplerPython(f); },
    Wheel(f) { return emitWheelPython(f); },
    TimingPulley(f) { return emitTimingPulleyPython(f); },
    Hinge(f) { return emitHingePython(f); },
    ProjectBox(f) { return emitProjectBoxPython(f); },
    PCBTray(f) { return emitPCBTrayPython(f); },
    Knob(f) { return emitKnobPython(f); },
    Foot(f) { return emitFootPython(f); },
    Gusset(f) { return emitGussetPython(f); },
    Handle(f) { return emitHandlePython(f); },
    ShaftHub(f) { return emitShaftHubPython(f); },
    Lid(f) { return emitLidPython(f); },
    RackGear(f) { return emitRackGearPython(f); },
    BatteryHolder(f) { return emitBatteryHolderPython(f); },
    DINRailClip(f) { return emitDINRailClipPython(f); },
    CableClip(f) { return emitCableClipPython(f); },
    GridfinityBin(f) { return emitGridfinityBinPython(f); },
    TSlotBracket(f) { return emitTSlotBracketPython(f); },
    Impeller(f) { return emitImpellerPython(f); },
    Auger(f) { return emitAugerPython(f); },
    BlowerWheel(f) { return emitBlowerWheelPython(f); },
    PaddleWheel(f) { return emitPaddleWheelPython(f); },
    PiCase(f) { return emitPiCasePython(f); },
};

/**
 * Generate build123d Python for a Gear feature — a spur-style gear with tapered
 * straight-flank teeth (v1; true involute flanks are a future refinement), an
 * optional centre bore, an optional bolt circle of holes, and optional
 * smooth-edge fillets. Pure function of the feature params; deterministic
 * (every number is cleanNum'd, the only runtime math is the per-tooth trig
 * loop). The whole build is wrapped in try/except that DEGRADES to a plain
 * bored disk, so even if the profile/extrude/fillet ever fails on the kernel
 * the document still compiles (the Casing emitter follows the same rule).
 *
 * Sits Align.MIN on Z (bottom face on Z=0, spanning 0..thickness), per the
 * project's Z-up primitive convention. Assigns n_<id> (the gear body).
 *
 * @param {object} f the Gear feature
 * @returns {string} python source assigning n_<f.id>
 */
export function emitGearPython(f) {
    const id = f.id;
    const p = f.params || {};
    const C = cleanNum;
    const z = Math.max(3, Math.round(Number(p.teeth) || 12));
    const mod = (Number(p.module) > 0) ? Number(p.module) : 2;
    const th = (Number(p.thickness) > 0) ? Number(p.thickness) : 10;
    const boreD = (Number(p.bore) > 0) ? Number(p.bore) : 0;
    const bc = Math.max(0, Math.round(Number(p.boltCount) || 0));
    const bcd = (Number(p.boltCircleDiameter) > 0) ? Number(p.boltCircleDiameter) : 0;
    const bhd = (Number(p.boltHoleDiameter) > 0) ? Number(p.boltHoleDiameter) : 3;
    const tf = (Number(p.toothFillet) > 0) ? Number(p.toothFillet) : 0;

    // Gear geometry (mm). pitch → tip(addendum) / root(dedendum) radii.
    const rp = mod * z / 2;
    const ra = rp + mod;                       // tip / outer radius
    const rf = Math.max(rp - 1.25 * mod, 0.5); // root radius
    const hp = Math.PI / (2 * z);              // half tooth angular width @ pitch
    const wr = 1.15 * hp;                      // root half-angle (wider)
    const wt = 0.6 * hp;                       // tip half-angle (narrower → taper)
    const ang = 2 * Math.PI / z;

    // Subtract a centre bore: a Cylinder spanning Z -1..th+1 (through-cut).
    const boreCut = (cx, cy) =>
        `n_${id} = n_${id} - Cylinder(${C(boreD / 2)}, ${C(th + 2)}, align=(Align.CENTER, Align.CENTER, Align.MIN)).moved(Location((${cx}, ${cy}, -1)))`;

    const L = [];
    L.push(`# ── Gear ${id} (${z} teeth, module ${C(mod)}mm, OD ${C(2 * ra)}mm, thickness ${C(th)}mm) ──`);
    L.push(`# Spur-style gear: tapered straight-flank teeth (v1), optional bore + bolt circle.`);
    L.push(`import math as _m`);
    L.push(`try:`);
    L.push(`    _pts_${id} = []`);
    L.push(`    _ang_${id} = ${C(ang)}`);
    L.push(`    for _k in range(${z}):`);
    L.push(`        _c = _k*_ang_${id}`);
    L.push(`        for (_r, _t) in ((${C(rf)}, _c-${C(wr)}), (${C(ra)}, _c-${C(wt)}), (${C(ra)}, _c+${C(wt)}), (${C(rf)}, _c+${C(wr)}), (${C(rf)}, _c+_ang_${id}/2.0)):`);
    L.push(`            _pts_${id}.append((_r*_m.cos(_t), _r*_m.sin(_t)))`);
    L.push(`    n_${id} = extrude(make_face(Polyline(*_pts_${id}, close=True)), amount=${C(th)})`);
    if (boreD > 0) L.push(`    ${boreCut('0', '0')}`);
    if (bc > 0 && bcd > 0) {
        L.push(`    _bcr_${id} = ${C(bcd / 2)}`);
        L.push(`    for _j in range(${bc}):`);
        L.push(`        _ba = 2*_m.pi*_j/${bc}`);
        L.push(`        n_${id} = n_${id} - Cylinder(${C(bhd / 2)}, ${C(th + 2)}, align=(Align.CENTER, Align.CENTER, Align.MIN)).moved(Location((_bcr_${id}*_m.cos(_ba), _bcr_${id}*_m.sin(_ba), -1)))`);
    }
    if (tf > 0) {
        // Smooth the vertical (Z-parallel) tooth/bore edges. Fillet can choke on
        // a busy edge set — keep it best-effort so a failure doesn't lose the gear.
        L.push(`    try:`);
        L.push(`        n_${id} = fillet(n_${id}.edges().filter_by(Axis.Z), radius=${C(tf)})`);
        L.push(`    except Exception:`);
        L.push(`        pass  # fillet skipped — gear still valid`);
    }
    L.push(`except Exception as _gear_err_${id}:`);
    L.push(`    # Gear build failed — fall back to a plain bored disk so the document still compiles.`);
    L.push(`    n_${id} = Cylinder(${C(ra)}, ${C(th)}, align=(Align.CENTER, Align.CENTER, Align.MIN))`);
    if (boreD > 0) L.push(`    ${boreCut('0', '0')}`);
    return L.join('\n');
}
...
export function emitDocument(doc, { topologyIndex = null, incremental = false } = {}) {
    ...
    // Phase G4 — incremental recompile (opt-in; default OFF so the canonical
    // compile path stays byte-identical and cannot regress). When on, each
    // body-producing feature is wrapped so the kernel reuses a memoised BREP
    // keyed by the feature's dependency hash (see _wrapCheckpoint + harness
    // _ckpt_*); only edited features + their downstream actually recompute.
    const depHashes = incremental ? featureDepHashes(doc) : null;
    ...
    for (const featureId of order) {
        ...
        try {
            ...
            lines.push(depHashes ? _wrapCheckpoint(statement, f.id, depHashes.get(f.id)) : statement);
        } catch (err) {
    ...
    const leafIds = order.filter(id => {
        ...
        // NOTE: BuildScript is NOT here — it now binds `n_<id> = _bs_run(...)`, so a script that
        // assigns `result` is a renderable leaf body; a helper-only script yields None and is
        // skipped by the kernel's None-guards (harness.py) rather than by being excluded here.
        if (['Sketch','SketchOnFace','Plane','Axis','Point','Parameter','Equation',
             ...
             'ImportedMesh','ReferenceImage'].includes(f.type)) {
            return false;
        }
        return true;
    });
    ...
    return {
        ...
        order,
        leafIds,
        depHashes: depHashes ? hashesToObject(depHashes) : null,
    };
...
/**
 * Phase G4 — wrap a feature's emitted statement in a kernel body-cache guard.
 * Only statements that actually bind the body var `n_<id>` are wrapped (a
 * comment-only / scene-only feature is returned untouched). On a cache hit the
 * expensive original statement is skipped entirely:
 *
 *     n_<id> = _ckpt_get("<hash>")
 *     if n_<id> is None:
 *         <original statement, indented>
 *         _ckpt_put("<hash>", n_<id>)
 *
 * Python `if` introduces no scope, so any helper vars the original statement
 * defined remain valid when it runs; downstream features only ever read
 * `n_<id>`, never those helpers, so skipping the block on a hit is safe.
 * Component placement is applied later (placeBodyExpr) on the result dict, not
 * here, so the cached body is placement-free and reused across moves.
 */
function _wrapCheckpoint(statement, fid, hash) {
    if (!hash) return statement;
    const v = `n_${fid}`;
    // Must bind the body var to be cacheable (skip comments / scene-only).
    if (!new RegExp(`(^|\\n)\\s*${v}\\s*=`).test(statement)) return statement;
    const indented = statement.split('\n').map((l) => `    ${l}`).join('\n');
    return `${v} = _ckpt_get(${py(hash)})\n`
        + `if ${v} is None:\n`
        + `${indented}\n`
        + `    _ckpt_put(${py(hash)}, ${v})`;
}
