...
// ── Insert-at-marker resolver (Phase B2) ─────────────────────────────────────
// Module-level (NOT per-store-instance) so it survives resetDocumentStore() the
// same way operations.js's active-component provider does — a fresh store after
// a document load keeps honouring the marker. The studio runtime installs a
// resolver `(doc) => index|null` that returns the featureOrder index a new
// feature should land at when a rollback marker is active, or null to append.
// Read at the commit choke point so EVERY add-feature path (operations, AI
// tools, batch) honours it without touching call sites.
let _insertIndexResolver = null;

/** Install the insert-at-marker resolver. Pass null to clear. */
export function setInsertIndexResolver(fn) {
    _insertIndexResolver = (typeof fn === 'function') ? fn : null;
}
...
export class DocumentStore {
    ...
    constructor(initial = null) {
        ...
        // Plan-graph — AI design-intent source of truth. Doc-level metadata (NOT
        // derived from the changelog), round-tripped as an OPAQUE POJO so the
        // document layer never imports the AI schema (src/lib/ai/plan/graph.js).
        if (initial && initial.planGraph && typeof initial.planGraph === 'object') {
            this.doc.planGraph = initial.planGraph;
        }
     ...
     */
    commit(change) {
        // Phase B2 — insert-at-marker. When a rollback marker is active and the
        // add-feature change didn't already carry an explicit insertAt, ask the
        // injected resolver where it should land in featureOrder. Null/append
        // preserves legacy behaviour bit-for-bit.
        if (change.kind === 'add-feature'
            && change.payload
            && (change.payload.insertAt === null || change.payload.insertAt === undefined)
            && _insertIndexResolver) {
            try {
                const at = _insertIndexResolver(this.doc);
                if (at !== null && at !== undefined) change.payload.insertAt = at;
    ...
    toJSON() {
        ...
        if (this.doc.planGraph && typeof this.doc.planGraph === 'object') {
            out.planGraph = this.doc.planGraph;
        }
    ...
    fromJSON(json) {
        ...
        // Lift the opaque plan-graph POJO (AI design intent) onto the live doc.
        if (v5json.planGraph && typeof v5json.planGraph === 'object') {
            refolded.planGraph = v5json.planGraph;
        }
        this.doc = refolded;
        this._redo.length = 0;
        this._selectedFeatureId = null;
