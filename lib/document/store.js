...
export class DocumentStore {
    ...
    constructor(initial = null) {
        ...
        this._selectedRefs = [];
        // Phase B2 — insert-at-marker. The studio runtime installs a resolver
        // (via setInsertIndexResolver) that returns the featureOrder index a new
        // feature should land at when a rollback marker is active, or null to
        // append. Lives here (not in operations.js) so EVERY add-feature commit
        // honours the marker without touching ~40 add* call sites, and so the
        // store stays runtime-agnostic (the resolver is injected, not imported).
        this._insertIndexResolver = null;
    ...
    /**
     * Phase B2 — install the insert-at-marker resolver. `fn(doc) → number|null`
     * returns the featureOrder index a new feature should be inserted at (when a
     * rollback marker is active) or null/undefined to append. Pass null to clear.
     */
    setInsertIndexResolver(fn) {
        this._insertIndexResolver = (typeof fn === 'function') ? fn : null;
    }
    ...
    commit(change) {
        ...
        if (change.kind === 'add-feature'
            ...
            && this._insertIndexResolver) {
            try {
                const at = this._insertIndexResolver(this.doc);
