...
export class DocumentViewportBridge {
    ...
    /**
     * Schedule a run after `debounceMs`. Repeated calls reset the timer so
     * rapid keystrokes (slider drag, …) coalesce into a single kernel call.
     */
    scheduleRun() {
        clearTimeout(this._debounceTimer);
        this._debounceTimer = setTimeout(() => this.runNow(), this.debounceMs);
    }
    ...
    _render(result) {
        ...
        try {
            this._loader.parse(result.glb, '', (gltf) => {
                ...
                // topology re-query. Multi-body docs end up with the same leaf
                // descriptor on every mesh today — refine when the kernel
                // emits a per-body mapping.
                const leafIds = (result.topology && Array.isArray(result.topology.nodes))
                    ...
                    : [];
                if (leafIds.length > 0) {
                    const leafId = leafIds[leafIds.length - 1];
                    const desc   = bodyRef(leafId);
                    wrap.userData.descriptor = desc;
                    wrap.userData.featureId  = leafId;
                    wrap.userData.leafFeatureIds = leafIds.slice();
                    gltf.scene.traverse((obj) => {
                        if (obj.isMesh) {
                            obj.userData.descriptor = desc;
                            obj.userData.featureId  = leafId;
                        }
                    });
                }
