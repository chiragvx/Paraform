...
// Verbose lifecycle logging on the commit/render hot path. Gated so it stays
// out of production bundles (Vite statically replaces import.meta.env.DEV with
// false in prod → the minifier drops the dead branch) and out of the Node test
// runner (import.meta.env is undefined there → falsy). Error/warn logging is
// intentionally left unconditional.
...
export class DocumentViewportBridge {
    ...
    constructor({ scene = null, store = null, executor = null, kernelClient = null, debounceMs = 150 } = {}) {
        ...
        // ── Render-only body visibility (Tier 1a) ────────────────────────────
        // Set of featureIds whose meshes are hidden in the scene. This is a
        // PURE render flag (mesh.visible = false) — the body stays in the
        // document, the kernel never re-runs. The runtime drives it via
        // `applyHiddenBodies(set)`; we also re-apply at the tail of every
        // scene (re)build so visibility survives a recompile. Always a Set.
        /** @type {Set<string>} */
        this._hiddenBodies = new Set();

        // ── Tier 1b — skip redundant scene rebuild on identical code ─────────
        // Fingerprint (`deflection|len|cyrb53`) of the emitted Python that
        // produced the CURRENTLY-APPLIED scene. When a fresh run emits the
        // same fingerprint AND the last apply succeeded, the geometry is
        // provably identical, so we skip the executor's parse + the full
        // scene teardown/rebuild entirely (rename / colour / reorder /
        // metadata edits all re-emit byte-identical Python). Set ONLY after a
        // successful apply; nulled on any error so a retry is never wrongly
        // skipped. `_pendingCodeHash` carries the fingerprint of the in-flight
        // run from the executor's synchronous `emit` event to its `executed`
        // event so the skip decision sees the right key.
        this._lastAppliedCodeHash = null;
        this._pendingCodeHash     = null;
        ...
        // ── EdgesGeometry memoization (Pass-2 hot-path) ──────────────────────
        // EdgesGeometry walks every triangle of every mesh and builds a fresh
        // BufferGeometry per body — the file comments above the deferred rAF
        // call this "the dominant cost on the render hot path." On a real
        // geometry edit the GLB is reparsed and the body meshes are rebuilt
        // fresh (that MUST stay — see _render), but in a multi-body doc most
        // bodies are geometrically unchanged, so re-deriving their edge overlay
        // is pure waste. We memoize the edge BufferGeometry keyed by a CONTENT
        // signature (`_meshEdgeSignature`) of the mesh's position+index buffers
        // — never by descriptor/featureId, which can be stale or ambiguous. On
        // a hit we reuse the cached geometry by wrapping it in a fresh
        // LineSegments (three.js allows many Object3Ds to share one
        // BufferGeometry — the transform lives on the object, so sharing is
        // safe). The body meshes themselves are NEVER grafted or reused.
        //
        //   _edgeGeomCache   sig → THREE.BufferGeometry   (each tagged
        //                    `userData.__edgeCached = true` so _clearRoot skips
        //                    disposing it).
        //   _edgeCacheUsed   sigs that the most-recent build actually consumed
        //                    — the eviction pass NEVER disposes a still-used
        //                    (i.e. live-in-scene) entry.
        /** @type {Map<string, THREE.BufferGeometry>} */
        this._edgeGeomCache = new Map();
        /** @type {Set<string>} */
        this._edgeCacheUsed = new Set();
        // Cache ceiling — LRU by Map insertion order. A small bounded GPU-memory
        // footprint; eviction is gated on liveness (see _evictEdgeCache).
        this._edgeCacheMax = 64;
        ...
        if (this.store && this.store.doc) {
            ...
            if (DEBUG) console.log('[bridge] constructed', { featureCount, kickInitialRender: featureCount > 0 });
    ...
    // ── Render-only body visibility ─────────────────────────────────────────

    /**
     * Apply a render-only hidden-body set. `hiddenSet` is a `Set<featureId>`;
     * every mesh whose `userData.featureId` is in the set gets
     * `mesh.visible = false` (and the rest are made visible). Edge-overlay
     * `LineSegments` are children of their owning mesh, so hiding the mesh
     * hides its edges automatically — but we still flip them explicitly in
     * case a future overlay parents elsewhere. A featureId can map to several
     * meshes spread across component subgroups; the traversal handles the
     * nesting.
     *
     * This is a pure scene flag — it NEVER recompiles. The runtime calls it
     * from the feature-tree eye toggle, and the bridge re-applies it at the
     * tail of every (re)build so visibility survives a regen.
     *
     * @param {Set<string>|null|undefined} hiddenSet
     */
    applyHiddenBodies(hiddenSet) {
        this._hiddenBodies = (hiddenSet instanceof Set) ? hiddenSet : new Set();
        this._reapplyHiddenBodies();
    }

    /**
     * Walk `_root` and set `obj.visible` for every mesh according to
     * `_hiddenBodies`. Called by `applyHiddenBodies` and at the tail of the
     * apply path so a recompile can't resurrect a hidden body. Cheap: one
     * traversal of the root group, no allocation.
     *
     * Edge overlays / other LineSegments tagged `__v4_overlay` inherit their
     * owning mesh's featureId only implicitly (they're children, so they're
     * culled with the parent), so we key purely on `featureId` and let
     * three.js' visibility inheritance hide descendants of a hidden mesh.
     *
     * @private
     */
    _reapplyHiddenBodies() {
        const root = this._root;
        if (!root) return;
        const hidden = this._hiddenBodies;
        // Empty set → make sure nothing is left stuck invisible from a prior
        // hide. We still need a traversal to clear stale flags.
        root.traverse((obj) => {
            if (!obj.isMesh) return;
            const fid = obj.userData && obj.userData.featureId;
            // Untagged meshes (fid undefined) are never hidden — `has(undefined)`
            // is false, so they stay visible. A tagged mesh is hidden iff its
            // featureId is in the set.
            obj.visible = !(fid && hidden.has(fid));
        });
    }
    ...
    dispose() {
        ...
        this._clearRoot();
        // _clearRoot deliberately skips cache-tagged edge geometries; the
        // bridge is going away for good, so free them here (the only place we
        // dispose live cache entries — `_evictEdgeCache` never touches the
        // current scene's entries, but on teardown nothing is "live" anymore).
        for (const geom of this._edgeGeomCache.values()) {
            try { if (geom && geom.dispose) geom.dispose(); } catch {}
        }
        this._edgeGeomCache.clear();
        this._edgeCacheUsed.clear();
        if (this._root.parent) this._root.parent.remove(this._root);
        this._unsubStore = null;
        this._unsubExecutor = null;
    }
    ...
    async runNow() {
        ...
        if (DEBUG) console.log('[bridge] runNow start', { featureCount });
        if (!doc || featureCount === 0) {
            ...
            this._lastResult = null;
            // Empty scene matches no applied code — invalidate so a later
            // re-add of previously-seen geometry rebuilds rather than skips.
            this._lastAppliedCodeHash = null;
            this._executedHash = null;
            this._notifyRender({ glb: null, empty: true });
            if (DEBUG) console.log('[bridge] runNow: empty doc, cleared root');
        ...
        try {
            ...
            if (DEBUG) console.log('[bridge] runNow done', {
                ok: result?.ok,
    ...
    _attachListeners() {
        ...
        this._unsubStore = this.store.subscribe((doc, eventLabel) => {
            if (DEBUG) console.log('[bridge] store change', { eventLabel, featureCount: Object.keys(doc?.features || {}).length });
        ...
        this._unsubExecutor = this.executor.subscribe((evt) => {
            if (evt.type === 'emit') {
                // Synchronous prelude to every run: the executor emits the
                // Python here, BEFORE the (possibly cached) network call. Stash
                // the fingerprint of the in-flight code so the `executed`
                // handler can compare it against the currently-applied scene.
                // Folding deflection into the key mirrors the executor's own
                // result-cache key (executor._resultCacheKey) — a tessellation
                // change at the same code must still force a rebuild.
                this._pendingCodeHash = this._codeFingerprint(evt.code);
                return;
            }
            if (evt.type === 'executed') {
                // ── Tier 1b skip ─────────────────────────────────────────────
                // Identical emitted code (same deflection) ⇒ byte-identical
                // geometry ⇒ the live scene is already correct. Skip the GLB
                // parse + full teardown/rebuild; only re-assert hidden bodies
                // (a hide toggle may have raced this run) and return. We still
                // refresh the cheap diagnostic/version bookkeeping above-the-
                // line below so consumers stay consistent, but we do NOT
                // re-fire onRender — geometry is unchanged, so re-running the
                // pick-proxy / overlay rebuild would defeat the whole point.
                const pending = this._pendingCodeHash;
                this._pendingCodeHash = null;
                if (pending && pending === this._lastAppliedCodeHash) {
                    ...
                    this._reapplyHiddenBodies();
                    if (DEBUG) console.log('[bridge] Tier 1b: code unchanged, skipped scene rebuild');
                    return;
                }
                this._executedHash = pending;
                ...
                // The run failed — the on-screen scene is still the LAST GOOD
                // one, so `_lastAppliedCodeHash` (which describes it) stays
                // valid. But the fingerprint we stashed for THIS (now failed)
                // run must not leak into a later `executed`, so drop it.
                this._pendingCodeHash = null;
    ...
    _render(result) {
        if (DEBUG) console.log('[bridge] _render called', {
            hasResult: !!result,
        ...
        if (!result || !result.glb || (result.glb.byteLength === 0)) {
            this._clearRoot();
            // Scene is now empty — no applied code describes it. Invalidate so
            // re-adding a body that re-emits previously-seen code rebuilds
            // instead of skipping against the empty scene.
            this._lastAppliedCodeHash = null;
            this._executedHash = null;
        ...
        try {
            this._loader.parse(result.glb, '', (gltf) => {
                ...
                requestAnimationFrame(() => {
                    ...
                    // Fresh per-build accounting of which cached signatures the
                    // scene now references. Reset here (inside the winning rAF,
                    // after the generation guard) so a superseded batch never
                    // clobbers it. The eviction pass at the tail consults this
                    // to avoid freeing a geometry the live scene still holds.
                    this._edgeCacheUsed = new Set();
                    for (const obj of meshesAwaitingEdges) {
                        ...
                        try {
                            // Content signature of the mesh's geometry. A cache
                            // hit reuses the memoized EdgesGeometry; a miss (or
                            // a null signature — missing/empty geometry) falls
                            // back to building one fresh, exactly as before.
                            const sig = _meshEdgeSignature(obj.geometry);
                            let edgesGeo = sig ? this._edgeGeomCache.get(sig) : null;
                            if (edgesGeo) {
                                // LRU touch — re-insert so this entry counts as
                                // most-recently-used for the bounded eviction.
                                this._edgeGeomCache.delete(sig);
                                this._edgeGeomCache.set(sig, edgesGeo);
                            } else {
                                edgesGeo = new THREE.EdgesGeometry(obj.geometry, 20);
                                if (sig) {
                                    // Tag so _clearRoot skips disposing it, then
                                    // memoize. Genuinely-uncacheable meshes (no
                                    // signature) stay un-tagged and dispose
                                    // normally — they're rebuilt every time.
                                    edgesGeo.userData = edgesGeo.userData || {};
                                    edgesGeo.userData.__edgeCached = true;
                                    this._edgeGeomCache.set(sig, edgesGeo);
                                }
                            }
                            if (sig) this._edgeCacheUsed.add(sig);
                            ...
                            // Many LineSegments may share one cached
                            // BufferGeometry — the transform rides the object,
                            // not the geometry, so this is safe.
                    ...
                    // Bound the cache. Only entries NOT consumed by this build
                    // are eviction candidates — anything in `_edgeCacheUsed` is
                    // referenced by a LineSegments currently in the scene.
                    this._evictEdgeCache();
                });
                ...
                // Re-assert render-only hidden bodies AFTER meshes are tagged
                // and reparented, so a body the user hid before this recompile
                // stays hidden (the fresh wrap would otherwise show it).
                this._reapplyHiddenBodies();
                // Commit the code fingerprint that produced this now-applied
                // scene. A later run emitting the same fingerprint will short-
                // circuit in the `executed` handler (Tier 1b). Set ONLY here,
                // on the successful-apply tail, so a failed/partial apply never
                // marks the scene as up-to-date.
                this._lastAppliedCodeHash = this._executedHash || null;
                this._executedHash = null;
                ...
                // Apply failed — invalidate the applied-code fingerprint so a
                // retry (same code) is NOT wrongly skipped by Tier 1b.
                this._lastAppliedCodeHash = null;
                this._executedHash = null;
                for (const fn of this._errorHooks) {
            ...
            // Synchronous parse/apply throw — same invalidation as above so the
            // next run rebuilds rather than skipping on a stale fingerprint.
            this._lastAppliedCodeHash = null;
            this._executedHash = null;
        }
    }
    ...
    /**
     * Fingerprint emitted Python for the Tier 1b skip. The key folds in the
     * tessellation deflection (a draft↔ultra change re-tessellates the SAME
     * code into different meshes, so it must NOT skip) and the code length
     * alongside the cyrb53 hash — exactly the shape the executor's own
     * result-cache key uses (`executor._resultCacheKey`), so the two stay in
     * lock-step. Reads the deflection off the executor when available.
     *
     * @param {string} code
     * @returns {string|null}
     * @private
     */
    _codeFingerprint(code) {
        if (typeof code !== 'string') return null;
        let deflection = 0.1;
        try {
            if (this.executor && typeof this.executor.getTessellationDeflection === 'function') {
                const d = this.executor.getTessellationDeflection();
                if (Number.isFinite(d) && d > 0) deflection = d;
            }
        } catch { /* default deflection */ }
        return `${deflection}|${code.length}|${_cyrb53(code)}`;
    }
    ...
    _clearRoot() {
        while (this._root.children.length) {
            ...
            try {
                child.traverse?.((o) => {
                    // CRITICAL: never dispose a geometry that lives in the edge
                    // cache. The cached EdgesGeometry is shared by reference —
                    // disposing it here would free GPU buffers a soon-to-be-
                    // reused (or still-live) LineSegments depends on, producing
                    // ghost/stale edges. The cache's own bounded eviction
                    // (`_evictEdgeCache`) is the sole owner of these disposes.
                    if (o.geometry && o.geometry.dispose &&
                        !(o.geometry.userData && o.geometry.userData.__edgeCached)) {
                        o.geometry.dispose();
                    }
                    if (o.material) {
    ...
    /**
     * Bound the edge-geometry cache to `_edgeCacheMax` entries, LRU by Map
     * insertion order (every hit re-inserts its entry, so the front of the
     * Map is the least-recently-used). The hard rule: an entry is an eviction
     * candidate ONLY if its signature is NOT in `_edgeCacheUsed` — i.e. no
     * LineSegments in the currently-displayed scene references it. Evicting a
     * live entry would free a geometry still on screen (ghost edges). If we
     * can't get under the cap without touching live entries, we leave the
     * cache slightly over the cap — a tiny bounded GPU-memory ceiling beats a
     * freed-in-use geometry.
     *
     * @private
     */
    _evictEdgeCache() {
        if (this._edgeGeomCache.size <= this._edgeCacheMax) return;
        // Iterate oldest-first (Map preserves insertion order).
        for (const [sig, geom] of this._edgeGeomCache) {
            if (this._edgeGeomCache.size <= this._edgeCacheMax) break;
            // Live entry — referenced by the current scene. Never dispose.
            if (this._edgeCacheUsed.has(sig)) continue;
            this._edgeGeomCache.delete(sig);
            try { if (geom && geom.dispose) geom.dispose(); } catch {}
        }
    }
...
// ── Emitted-code fingerprint (Tier 1b) ────────────────────────────────────────

/**
 * cyrb53 — a fast, well-distributed 53-bit non-cryptographic string hash.
 * Replicated from `lib/document/executor.js` (where it's module-private) so
 * the bridge can fingerprint emitted code WITHOUT re-emitting it. Keeping the
 * algorithm identical means the bridge's Tier 1b key and the executor's
 * result-cache key would agree on the hash component if compared — they fold
 * in the same `deflection|length|cyrb53(code)` shape. Not security-sensitive;
 * chosen for speed and a collision rate negligible at this scale.
...
// ── EdgesGeometry memo signature ──────────────────────────────────────────────

/**
 * Content signature of a mesh BufferGeometry, used to key the bridge's
 * EdgesGeometry cache. EdgesGeometry is a pure function of the `position`
 * attribute + `index` (+ the fixed 20° threshold), so two meshes with
 * byte-identical position/index buffers produce identical edge overlays and
 * can share one cached BufferGeometry.
 *
 * The signature MUST be content-derived (never descriptor/featureId — those
 * survive a real geometry edit and would yield a stale-edge collision). We
 * fold in:
 *   - vertex count + index count + array lengths (cheap discriminators),
 *   - a cyrb53 over the FULL position float buffer (read as its byte view so
 *     NaN/-0 bit patterns still contribute),
 *   - a cyrb53 over the FULL index buffer when present.
 * The full-buffer pass (not a sample) keeps the collision rate negligible —
 * a near-zero collision rate is required here because a false hit shows as
 * ghost/stale edges on a geometrically-different body.
 *
 * Returns `null` when the geometry or its position attribute is missing or
 * empty — the caller then builds a fresh, un-cached EdgesGeometry. Never
 * throws.
 *
 * @param {THREE.BufferGeometry} geometry
 * @returns {string|null}
 */
function _meshEdgeSignature(geometry) {
    try {
        if (!geometry) return null;
        const pos = geometry.getAttribute && geometry.getAttribute('position');
        if (!pos || !pos.array || pos.count === 0) return null;
        const posArr = pos.array;
        // Hash the raw bytes of the position buffer so every bit (incl. sign /
        // NaN patterns) contributes — _cyrb53 takes a string-like with
        // `.length` + `.charCodeAt`, and a Uint8 view over the float buffer
        // satisfies a char-code walk byte-for-byte.
        const posBytes = new Uint8Array(posArr.buffer, posArr.byteOffset, posArr.byteLength);
        const posHash  = _cyrb53Bytes(posBytes);
        const idx = geometry.getIndex && geometry.getIndex();
        let idxLen = 0;
        let idxHash = 0;
        if (idx && idx.array && idx.count > 0) {
            const idxArr = idx.array;
            idxLen = idx.count;
            const idxBytes = new Uint8Array(idxArr.buffer, idxArr.byteOffset, idxArr.byteLength);
            idxHash = _cyrb53Bytes(idxBytes);
        }
        // vertexCount | posBufLen | posHash | indexCount | idxHash
        return `${pos.count}|${posArr.length}|${posHash}|${idxLen}|${idxHash}`;
    } catch {
        return null;
    }
}
...
/**
 * cyrb53 over a byte view (Uint8Array). Same algorithm as `_cyrb53` but reads
 * `[i]` instead of `charCodeAt(i)` so it can hash a TypedArray's raw bytes
 * directly without a string copy. Public-domain (bryc).
 */
function _cyrb53Bytes(bytes, seed = 0) {
    let h1 = 0xdeadbeef ^ seed;
    let h2 = 0x41c6ce57 ^ seed;
