...
export function applyChange(doc, change, opts = {}) {
    ...
    switch (kind) {
        ...
        case CHANGE_KIND.UPDATE_CONNECTOR: {
            // Lock enforcement (see CLAUDE.md "The connector contract") lives in
            // the op layer — operations.js `updateConnector` refuses to commit a
            // patch when the record is locked. The fold applier is intentionally
            // not the auth checkpoint: change records are infrastructure data and
            // do not carry the `force` flag, so a lock check here would break the
            // legitimate force-cleanup paths (replaceComponent).
            ...
            if (patch && typeof patch === 'object') {
                for (const k of Object.keys(patch)) {
                    if (k === 'locked') continue; // locked is set at creation, never patched
        ...
        case CHANGE_KIND.REMOVE_CONNECTOR: {
            // Lock check is in operations.js (see UPDATE_CONNECTOR comment).
